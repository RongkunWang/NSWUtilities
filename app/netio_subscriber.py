...
def options():
    ...
    parser.add_argument(
            "-D",
            dest="directory",
            default = "/atlas/oks/tdaq-09-04-00/muons/segments/NSW/ELinks-core",
            type=str,
...
def port_guess(elinks):
    """ Guess the felix port based on the elink given in decimal."""
    deviceNumber = [
        (i >> 11) & 0x3 for i in elinks
    ]  # extract Z from 0xYYZYY represented as a decimal
    fate = False
    for dN, elink in zip(deviceNumber, elinks):
        if dN > 3:
            (f"The inputted elink {hex(elink)} gives an invalid device number {dN}")
            fate = True
    if fate:
        fatal(
            "This program does not support the device numbers of the inputted elinks written above."
        )
    return [12350 + i for i in deviceNumber]
...
def main():
    ...
    elinks = []
    hostnames = []
    outdir = ops.o if ops.o else "netio_cat_elink_logs"
    ...
    encoding = ops.e
    ports = ops.p if ops.p else port_guess(elinks)
    ...
    jobs = []
    l_output = []
    for hostname, port, elink in zip(hostnames, ports, elinks):  # decimal,
        elink_one_server = elink & 0xffff
        flxid = elink >> 16
        path = os.path.join(outdir, f"flx_{flxid}.port{port}.elink_{elink_one_server}_0x{elink_one_server:x}.rawElink_{elink}_0x{elink:x}.log")
        netio = (f"netio_cat subscribe -H {hostname} -p {port} -e {encoding} -t {elink}")
        print(netio)
        netio = netio.split()
        print(netio)
